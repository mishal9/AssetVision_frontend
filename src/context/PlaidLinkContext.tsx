...
interface PlaidLinkContextType {
  ...
  open: (onSuccess?: (publicToken: string, metadata: PlaidMetadata) => void, onExit?: () => void) => void;
  ready: boolean;
...
export const PlaidLinkProvider = ({ children }: { children: ReactNode }) => {
  ...
  const successHandlerRef = useRef<((publicToken: string, metadata: PlaidMetadata) => void) | null>(null);
  ...
  const open = useCallback((onSuccess?: (publicToken: string, metadata: PlaidMetadata) => void, onExit?: () => void) => {
